...
// 5-section business website copy: hero, about, 6 feature cards. Used by the
// Landing Page module's business-site generator (replaces the old blog-style
// generator entirely).
const FEATURE_ICONS = [
...
async function generateBusinessSite({ domain, industry }) {
  const industryLine = industry
    ? `The business is in the "${industry}" industry — every section's wording must clearly fit that industry.`
    : `Keep the wording generic and brand-safe.`;

  const prompt = `You are a professional website copywriter. Write the copy for a 5-section marketing website homepage for the business/brand "${domain}".
${industryLine}

Return ONLY a JSON object with these exact keys:
{
  "hero": {
    "heading": "short, punchy hero headline (max 10 words)",
    "subheading": "1-2 sentence supporting line",
    "cta": "call-to-action button text (2-4 words)"
  },
  "about": {
    "heading": "short heading for the About Us section (max 6 words)",
    "content": "a paragraph of ABOUT 50 WORDS about the business, written in first person plural (we/our)"
  },
  "features": [
    { "icon": "pick ONE of: ${FEATURE_ICONS.join(", ")}", "heading": "short feature title (max 5 words)", "text": "a benefit-focused description of ABOUT 20 WORDS" }
  ],
  "imageKeyword": "2-3 words describing the ideal stock photo for this business's hero background"
}
The "features" array MUST contain exactly 6 items, each with a DIFFERENT icon from the list.`;
  ...
  try {
    ...
    if (!Array.isArray(parsed.features) || parsed.features.length < 6) {
      throw new Error("AI response missing 6 features");
    }
    return parsed;
  } catch (e) {
    console.warn("Business site AI generation failed, using fallback:", e.message);
    return fallbackBusinessSite(domain, industry);
  }
}

function fallbackBusinessSite(domain, industry) {
  const brand = domain || "your site";
  const ind = (industry || "").trim() || "our industry";
  const featureCopy = [
    ["bolt", "Fast & Reliable", `Built to keep pace with ${ind} demands, our platform stays fast and dependable so you never lose momentum during busy periods.`],
    ["shield-halved", "Secure by Design", "Your data and your customers' trust are protected at every step, with security built into the foundation of everything we do."],
    ["headset", "Real Support", "A real team is here to help whenever you need it, answering questions quickly and treating every customer like a priority."],
    ["chart-line", "Built to Grow", `Flexible enough to scale as your ${ind} business grows, adding new capabilities exactly when you need them without any hassle.`],
    ["thumbs-up", "Trusted by Many", "Join a growing list of happy customers who rely on us daily, drawn to consistent quality and a service they can trust."],
    ["gem", "Quality First", "Every detail is crafted with care and attention to quality, because we believe small things make the biggest difference for our customers."]
  ];
  return {
    hero: {
      heading: `Welcome to ${brand}`,
      subheading: `Everything you need for ${ind}, in one place.`,
      cta: "Get Started"
    },
    about: {
      heading: "About Us",
      content: `At ${brand}, we're dedicated to making ${ind} simple, effective, and genuinely enjoyable for every customer we serve. Our experienced team blends deep industry knowledge with modern tools and a hands-on approach, so you always get thoughtful solutions built around your real needs, not one-size-fits-all promises made by everyone else today.`
    },
    features: featureCopy.map(([icon, heading, text]) => ({ icon, heading, text })),
    imageKeyword: ind === "our industry" ? domain : ind
  };
}

module.exports = { generatePageContent, generateBlogContent, generateBusinessSite };
