    ...
    <div className="min-h-screen bg-white text-slate-900">
      ...
      <header className="border-b border-slate-100 sticky top-0 bg-white/90 backdrop-blur z-10">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="font-extrabold text-slate-900 text-lg tracking-tight flex items-center">
            <i className="fa-solid fa-layer-group mr-2" style={{ color: "#059669" }} />
            Lander Generation
          </div>
      ...
      <section className="relative overflow-hidden" style={{ background: "#0f172a" }}>
        <div className="absolute inset-0 opacity-20" style={{
          backgroundImage: "radial-gradient(circle at 20% 20%, #059669 0, transparent 45%), radial-gradient(circle at 80% 0%, #059669 0, transparent 40%)"
        }} />
        <div className="relative max-w-5xl mx-auto px-4 py-24 md:py-32 text-center">
          <span className="inline-flex items-center gap-2 text-xs font-semibold text-emerald-300 bg-emerald-500/10 border border-emerald-500/30 rounded-full px-3 py-1 mb-6">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link to="/register" className="bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-3 px-8 rounded-lg transition-colors shadow-lg shadow-emerald-500/30">
              Get started free
            </Link>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {MODULES.map(m => (
            <div key={m.title} className="card hover:shadow-md transition-shadow">
              <div className="w-12 h-12 rounded-lg flex items-center justify-center mb-4" style={{ background: "#ecfdf5" }}>
                <i className={`${m.icon} text-lg`} style={{ color: "#059669" }} />
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            {STEPS.map((s, i) => (
              <div key={s.title} className="relative text-center">
                ...
                <div className="text-xs font-semibold text-emerald-500 mb-1">STEP {i + 1}</div>
      ...
      <section style={{ background: "#1e293b" }}>
        <div className="max-w-4xl mx-auto px-4 py-16 text-center">
          ...
          <Link to="/register" className="inline-block bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-3 px-8 rounded-lg transition-colors">
            Get started free
          </Link>
        </div>
      </section>
      ...
      <footer className="border-t border-slate-100">
        <div className="max-w-6xl mx-auto px-4 py-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <p className="text-xs text-slate-400">© {new Date().getFullYear()} Lander Generation. All rights reserved.</p>
        </div>
      </footer>
    </div>
  );
